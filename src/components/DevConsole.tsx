...
export default function DevConsole({ logs, onClearLogs }: DevConsoleProps) {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-fluent-dark-bg text-gray-100 font-mono select-text animate-fade-in" id="dev-console-wrapper">
      {/* Console Top Header */}
      <div className="p-4 border-b border-fluent-dark-border flex items-center justify-between select-none">
        <div className="flex items-center space-x-2">
          <div className="w-2.5 h-2.5 rounded-full bg-red-500"></div>
          <div className="w-2.5 h-2.5 rounded-full bg-yellow-500"></div>
          <div className="w-2.5 h-2.5 rounded-full bg-green-500"></div>
          <span className="font-bold text-xs uppercase tracking-wider text-gray-300 ml-2">Microsoft Graph API Request Logs (DevConsole)</span>
        </div>
        <button
          onClick={onClearLogs}
      ...
      {logs.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center select-none" id="empty-state-console">
          ...
          <h4 className="text-xs font-bold text-gray-400 mt-4">Console Ready & Listening</h4>
          <p className="text-[11px] text-gray-500 max-w-sm mt-1 leading-normal">
            Browse folders, upload files, or trigger CRUD operations to inspect raw Microsoft Graph HTTP requests/responses, status codes, performance metrics, and copyable PowerShell or cURL queries.
          </p>
        </div>
      ) : (
        <div className="flex-1 flex min-h-0 divide-x divide-fluent-dark-border">
          ...
          <div className="w-80 flex flex-col min-h-0 select-none bg-black/20">
            <div className="p-3 bg-black/40 border-b border-fluent-dark-border text-[9px] font-bold text-gray-400 tracking-wider uppercase">
              HTTP Operations Log
            </div>
              ...
              <div className="flex bg-black/20 p-1 border-b border-fluent-dark-border select-none">
                {(["request", "response", "code"] as const).map((tab) => (
                  <button
                    ...
                    className={`px-4 py-1.5 text-[10px] font-bold rounded-sm capitalize transition-all ${
                      activeTab === tab
                    ...
                    <div>
                      ...
                      <div className="relative mt-1">
                        <button
                          ...
                          className="absolute right-2 top-2 bg-white/5 hover:bg-white/10 text-[9px] font-bold px-2 py-1 rounded-sm border border-fluent-dark-border hover:text-white transition-colors"
                        >
                          Copy Body
                        </button>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1.5 select-none">
                        ...
                        <button
                          ...
                          className="bg-fluent-brand/10 hover:bg-fluent-brand/20 text-fluent-brand text-[9px] font-bold px-2.5 py-1 rounded-sm border border-fluent-brand/20 hover:border-fluent-brand/40 transition-all"
                        >
                          Copy Script
                        </button>
                      </div>
